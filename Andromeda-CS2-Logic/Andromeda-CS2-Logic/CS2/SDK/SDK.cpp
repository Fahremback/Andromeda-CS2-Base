...
namespace SDK
{
	...
	static CGlobalVarsBase* g_pGlobalVarsShared = nullptr;

	void Interfaces::SetSharedState(::SharedState* state)
	{
		if (!state) return;
		
		g_pEngineToClient = state->sdk.engine;
		g_pGameEntitySystem = state->sdk.entitySystem;
		g_pSchemaSystem = state->sdk.schemaSystem;
		g_pSource2Client = state->sdk.source2Client;
		g_pLocalize = state->sdk.localize;
		g_pSoundOpSystem = state->sdk.soundOpSystem;
		g_pBaseFileSystem = state->sdk.baseFileSystem;
		g_pMaterialSystem2 = state->sdk.materialSystem2;
		g_pEngineCvar = state->sdk.engineCvar;
		g_pInputSystem = state->sdk.inputSystem;
		g_pGlobalVarsShared = state->sdk.globalVars;
	}
...
GetGameEntitySystemPointer:;
	...
	{
		if (g_pGlobalVarsShared) return g_pGlobalVarsShared;
